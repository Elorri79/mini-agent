 ...
 */
export async function skill_analyze_project(messages, question) {
    const results = [];
 ...
 */
export async function skill_debug_error(messages, errorMessage, question) {
    const results = [];
 ...
 */
export async function skill_resolve_dependencies(messages, question) {
    const results = [];
    // 1. Leer package.json
    const pkgContent = await readFileTool({ path: './package.json' });
 ...
 */
export async function executeSkill(skillName, messages, args) {
    const question = args.question || 'Analiza esto';
