...
export async function readFileTool(args: { path: string }): Promise<string> {
    try {
        ...
        const truncated =
            data.length > CONFIG.MAX_FILE_SIZE
 ...
 */
export async function extractPackageJsonDeps(filePath: string): Promise<string> {
    try {
        ...
        return (
            `El archivo package.json declara como dependencias: ` +
 ...
 */
export async function searchInFileTool(args: { path: string; pattern: string }): Promise<string> {
    try {
        const abs = path.resolve(process.cwd(), args.path);
        ...
        return (
            `Encontradas ${matches.length} coincidencias de "${args.pattern}" en ${abs}:\n` +
 ...
 */
export async function runCommandTool(args: { command: string }): Promise<string> {
    try {
        ...
        const truncated =
            result.length > CONFIG.MAX_FILE_SIZE
